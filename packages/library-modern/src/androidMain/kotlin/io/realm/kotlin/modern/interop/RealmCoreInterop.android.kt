...
/**
 * Android implementation of Modern Realm Interop Layer.
 * 
 * This implementation uses the modern RealmCoreAdapter to bridge with
 * the native realm-core C++ library through optimized JNI bindings.
 */
public actual object RealmCoreInterop {
    
    private val adapter = RealmCoreAdapter()
    
    // Fallback simulation mode for development/testing
    private val simulationMode = System.getProperty("realm.simulation.mode", "false").toBoolean()
    
    // Legacy handles for simulation mode
    private val handleCounter = AtomicLong(2000L)
    internal val databaseHandles = ConcurrentHashMap<Long, AndroidRealmDatabase>()
    internal val transactionHandles = ConcurrentHashMap<Long, AndroidRealmTransaction>()
    internal val objectHandles = ConcurrentHashMap<Long, AndroidRealmObject>()
    internal val queryHandles = ConcurrentHashMap<Long, AndroidRealmQuery>()
    internal val resultsHandles = ConcurrentHashMap<Long, AndroidRealmResults>()
    internal val listenerHandles = ConcurrentHashMap<Long, AndroidRealmListener>()
    ...
    public actual fun openDatabase(config: RealmConfiguration): RealmDatabaseHandle {
        return if (simulationMode) {
            ...
            openDatabaseSimulation(config)
        } else {
            // Use modern adapter with real realm-core connection
            try {
                adapter.openDatabase(config)
            } catch (e: Exception) {
                // Fallback to simulation mode if native connection fails
                PlatformUtils.logPlatformError("Native realm-core not available, using simulation mode", e)
                openDatabaseSimulation(config)
            }
        }
    }
    
    private fun openDatabaseSimulation(config: RealmConfiguration): RealmDatabaseHandle {
        ...
        val database = AndroidRealmDatabase(
            handle = handle,
            path = dbPath,
            config = config
        )
        ...
        PlatformUtils.logPlatformError("Opened Realm database (simulation mode) at: $dbPath", null)
    ...
    public actual fun beginWrite(handle: RealmDatabaseHandle): RealmTransactionHandle {
        ...
        val transaction = AndroidRealmTransaction(
    ...
    public actual fun createObject(
        ...
        val realmObject = AndroidRealmObject(
            handle = objectHandle,
            className = className,
    ...
    public actual fun executeQuery(queryHandle: RealmQueryHandle): RealmResultsHandle {
        ...
        val results = AndroidRealmResults(
    ...
    public actual fun addChangeListener(
        ...
        val listener = AndroidRealmListener(
    ...
    private fun generateHandle(): Long = handleCounter.incrementAndGet()
    
    private fun matchesFilters(obj: AndroidRealmObject, filters: List<QueryFilter>): Boolean {
...
internal data class AndroidRealmDatabase(
    val handle: Long,
    val path: String,
    val config: RealmConfiguration
)

internal data class AndroidRealmTransaction(
    val handle: Long,
    val databaseHandle: Long
)

internal data class AndroidRealmObject(
    val handle: Long,
    val className: String,
    val properties: MutableMap<String, Any?>
)

internal data class AndroidRealmQuery(
    val handle: Long,
    val className: String,
    val filters: MutableList<QueryFilter>
)

internal data class AndroidRealmResults(
    val handle: Long,
    val objects: List<Long>
)

internal data class AndroidRealmListener(
    val handle: Long,
