...
/**
 * JVM implementation of Modern Realm Interop Layer.
 * 
 * This implementation provides a simplified realm-core interface for JVM targets.
 * In a production environment, this would use JNI to communicate with the actual
 * realm-core C++ library.
 */
public actual object RealmCoreInterop {
    
    private val handleCounter = AtomicLong(1000L)
    internal val databaseHandles = ConcurrentHashMap<Long, JvmRealmDatabase>()
    internal val transactionHandles = ConcurrentHashMap<Long, JvmRealmTransaction>()
    internal val objectHandles = ConcurrentHashMap<Long, JvmRealmObject>()
    internal val queryHandles = ConcurrentHashMap<Long, JvmRealmQuery>()
    internal val resultsHandles = ConcurrentHashMap<Long, JvmRealmResults>()
    internal val listenerHandles = ConcurrentHashMap<Long, JvmRealmListener>()
    ...
    public actual fun beginWrite(handle: RealmDatabaseHandle): RealmTransactionHandle {
        ...
        val transaction = JvmRealmTransaction(
    ...
    public actual fun createObject(
        ...
        val realmObject = JvmRealmObject(
            handle = objectHandle,
            className = className,
    ...
    public actual fun addResultsListener(
    ...
    public actual fun addObjectListener(
        objectHandle: RealmObjectHandle,
    ...
    public actual fun addDatabaseListener(
    ...
    public actual fun removeListener(listenerHandle: RealmListenerHandle) {
    ...
    private fun matchesFilters(obj: JvmRealmObject, filters: List<QueryFilter>): Boolean {
...
internal data class JvmRealmDatabase(
    val handle: Long,
    val path: String,
    val config: RealmConfiguration
)

internal data class JvmRealmTransaction(
    val handle: Long,
    val databaseHandle: Long
)

internal data class JvmRealmObject(
    val handle: Long,
    val className: String,
    val properties: MutableMap<String, Any?>
)

internal data class JvmRealmQuery(
    val handle: Long,
    val className: String,
    val filters: MutableList<QueryFilter>
)

internal data class JvmRealmResults(
    val handle: Long,
    val objects: List<Long>
)

internal data class JvmRealmListener(
    val handle: Long,
    val callback: Any
)
